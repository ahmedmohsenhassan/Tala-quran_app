...
class QuranPageHelper {
  ...
  /// الحصول على رقم آخر صفحة للجزء
  static int getJuzEndPage(int juzNumber) {
    if (juzNumber >= 30) return 604;
    return getPageForJuz(juzNumber + 1) - 1;
  }

  /// الحصول على عدد الآيات لكل سورة
  static int getAyahCount(int surahNumber) {
    if (surahNumber < 1 || surahNumber > 114) return 0;
    return surahAyahCounts[surahNumber - 1];
  }

  /// الحصول على رقم السورة من اسمها البسيط
  static int getSurahNumberByName(String name) {
    return surahNames.indexOf(name) + 1;
  }
